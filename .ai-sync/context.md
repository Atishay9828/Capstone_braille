...
## [2026-09-01 evening] - Claude Code (Mridul's fork)
**Task:** Fix the braille cell proportions, undo a bad arm thickening, repair the cam polyhedron, and settle the CGAL-vs-Manifold question.
**Changes:** mech_layout.scad col_spacing 4.8 -> 2.6 and link_thickness back to 1.0 (arm_h stays 3.0); linkage.scad gained a nub-vs-bore assert; braille_cam.scad build_track_polyhedron now emits triangles instead of non-planar quads. All STLs re-exported on OpenSCAD 2026 nightly with --backend=CGAL.
**Status:** applied and verified. Firmware homing and the G-code re-slice are still open.
**Notes:** THE CELL WAS TWICE AS WIDE AS BRAILLE - col_spacing 4.8 against a 2.34 standard, rows 2.6. Braille pitch is SQUARE; a cell looks tall because it is 2 cols x 3 rows. Narrowing the columns to 2.6 was FREE because every clearance in the cluster is set by the closer rows - spring wall 0.40, arm gap 1.60, dome gap 1.10, all identical before and after. Cell is now a uniform 1.111x enlargement instead of 2.05x in one axis only. Zero clashes across all 15 linkage pairs.
TRUE 2.34 IS BLOCKED BY THE SPRING: its bore is 2.2mm and a 0.4mm wall forces pitch >= 2.6. Needs a spring under 1.74mm OD, or the spring off the dot axis entirely.
MY ARM THICKENING WAS WRONG AND MRIDUL CAUGHT IT IN THE RENDER. link_thickness is the extrusion depth, so it is also the NUB depth, and the nub passes a 1.4mm bore - at 2.0 it could not enter the hole. Also unnecessary: stiffness is height cubed, so 1.0 x 3.0 gives 27x with nothing getting wider. Asserted now.
