...
| **M4 motor ear screws** | 0 | ❌ **DO NOT BUY** (2026-08-31) | ⚠️ **The current stack has no motor screws.** The motor dropped onto the box floor on 2026-08-26, so its ears are at z=37 while this plate's pilots are at z=41 — 4mm apart, no screw can span them. `base_plate.scad` records this as deliberate: the seat carries the load and the wire block carries the torque. The earlier M4×5-brass line is withdrawn; the brass argument is also moot, because there is no longer a steel screw near the magnet orbit. Revisit only if retention bosses come back in the R-07 pass. |
...
No panel-mount jack is required for the current revision; use the owned pigtail and prove its cable-only slot first.
...
| 2 | Smallest pigtail cable slot that passes both insulated leads without damage | `power_cable_slot_w/h` in the pod lid; use the 5x3 / 6x4 / 7x5 coupon |
