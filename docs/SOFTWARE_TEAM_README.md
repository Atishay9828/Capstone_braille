...
```python
# v7.0 mapping. Which cam track drives which dot changed when the six
# linkage feet were spread around the disc, so this table is the ONLY
# correct one — do not use `1 << (dot-1)`.
DOT_TO_BIT = {1: 3, 2: 2, 3: 1, 4: 4, 5: 5, 6: 0}

def cell_to_cam(cell):            # cell = tuple of dots, e.g. (1, 4)
    value = 0
    for dot in cell:
        value |= 1 << DOT_TO_BIT[dot]
    return value                   # 0..63
```
Examples: `()` (blank) → `0`; `(1,)` → `8`; `(1,4)` → `24` (letter `c`).

> ⚠️ **LOCKED 2026-07-26 — this table is generated by the hardware layout.**
> The cam's innermost track physically cannot carry a fast-toggling bit, and the
> six feet are spread 60° apart around the disc, so dot→bit is a lookup, not a
> formula. The authority is `dot_track` in `cad/scad/mech_layout.scad` combined
> with `bit = 5 - track`. If a printed cell shows the wrong dots, check here first.
